                        ...
                        <div className="item-row" key={index}>
                            ...
                            {this.state.editUser === '0' ? (
                                <div className="column column-3">
                                    <input className="form-control" type="text" value={user.phone}
                                           onChange={this.handleChange.bind(this, index, 'phone')}
                            ...
                            {this.state.editUser === '0' ? (
                                <div className="column column-4">
                                    <Select size="large" value={user.role}
                                            onChange={this.handleChange.bind(this, index, 'role')}
                                            style={{width: "100%"}}>
                                        <Option value="">Select Role</Option>
                                        <Option value="District Officer">District Officer</Option>
                                        <Option value="DHS (Directorate of Health Services)">DHS (Directorate of Health
                                            Services)</Option>
                            ...
                            {this.state.editUser === user._id ? (
                                <div className="column column-5">
                                    <Select mode="multiple" size="large" value={user.districts}
                                            onChange={this.handleChange.bind(this, index, 'districts')}
                                            style={{width: "100%"}}>
                                        <Option value="All">All</Option>
                                        {this.state.districts.map((district, dIndex) => {
                                            return (
                                                <Option value={district.name} key={dIndex}
                                                        disabled={user.districts.indexOf('All') > -1 ? true : false}>{district.name}</Option>
                                            )
                                        })}
                                    </Select>
                                </div>
                            ...
                            {this.state.editUser === user._id ? (
                                <div className="column column-6">
                                    <Select size="large" value={user.status}
                                            onChange={this.handleChange.bind(this, index, 'status')}
                                            style={{width: "100%"}}>
                            ...
                            <div className="column column-7">
                                {this.state.editUser === user._id ? (
                                    <button className="btn column-btn"
                                            onClick={this.saveUser.bind(this, index, user)}>Save</button>
                                ) : (
                                    <button className="btn column-btn" disabled={this.state.editUser}
                                            onClick={this.editUser.bind(this, index, user._id)}>Edit</button>
                                )}
                            </div>
                        </div>
                    )
