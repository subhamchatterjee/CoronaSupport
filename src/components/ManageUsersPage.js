...
export default class ManageUsersPage extends Component {
	...
	addUser = () => {
		...
		if (users.length) {
			if (!users[users.length - 1].fullName || !users[users.length - 1].email || !users[users.length - 1].phone || !users[users.length - 1].role || !users[users.length - 1].districts.length || !users[users.length - 1].status) valid = false;
		}
	...
	saveUser = (index, user) => {
		...
		if (parseInt(user._id) === 0) {
			if (!user.fullName) error = 'fullName';
